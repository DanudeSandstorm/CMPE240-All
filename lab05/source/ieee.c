...
IEEE_FLT IeeeMult(IEEE_FLT a, IEEE_FLT b) {
	...
	exponent -= 127; //remove redudent bias
...
IEEE_FLT IeeeAdd(IEEE_FLT a, IEEE_FLT b) {
	...
	//Normilize mantissia if exponents are not equal
	//Make the lower exponent equal to the higher exponent
	//shift the mantissa of corresponding number right 1
	uint32_t shift = 1; //Used for re-normilization
	...
	if ((sign1 ^ sign2) == 1) {
		...
		//Subtraction by the same number is 0
		if (mantissa1 == mantissa2) {
			return 0x0;
		}
		//Subtract higher from lower and set sign of higher
		...
		//If both negative sign is negative
		if (sign1 == 1 && sign2 == 1) {
			sign = 0x1;
		}
	}
